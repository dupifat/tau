...
`read` range operations use inclusive `start_line` and `end_line` fields. `edit` range operations use boundary `after_line` and `before_line` fields.
...
`edit` is line-oriented. Each edit entry must include `after_line`, `before_line`, `newText`, and `guard`; it replaces the original lines between those boundaries with `newText` verbatim. `after_line: 0` means the start of the file, and `before_line: total_lines + 1` means EOF. Empty insertion ranges use `before_line == after_line + 1`; non-empty replacements cover `after_line + 1` through `before_line - 1`. All edit ranges use the original file numbering as if applied simultaneously, so the tool must reject overlapping ranges before changing the file. Unlike `read`, `edit` must not clip boundary ranges: `after_line` must be at most `total_lines`, `before_line` must be at most `total_lines + 1`, and `before_line` must be greater than `after_line`.
`edit` supports file creation: missing files are treated as empty, and missing parent directories are created only after the request validates. To create a file, insert with `after_line: 0`, `before_line: 1`. The model-visible result should stay minimal: `replacements`, `changed`, `new_max_valid_start_line`, and `total_bytes`; `new_max_valid_start_line` is after-edit state and must not be confused with original range validation.
