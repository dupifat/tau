...
`script` is required. Relative paths are resolved by the extension process current working directory; absolute paths are preferred. `vars` is passed to `init(config)` as JSON-compatible data, along with `state_dir` when the harness provides one.
...
`on_event(event, meta)` is optional. `meta.seq` and `meta.recorded_at` are present when Tau supplies them. The extension acknowledges sequenced deliveries after the callback attempt, even if the script errors.
...
Host functions are available after `init` succeeds, not during `init`. This keeps broken init scripts inert.
